...
for cf in cf_values:
    ...
    n_models = len(models)
    thr = 0.9   # The recall for true positives ### TODO: make this a input for the script
...
for i in range(len(model_to_use_with_cf)):
    ...
    for _, recall, thresholds in scc:
        # Getting the index positions for where the recall is greater than 0.90 to append the threshold value that got it
        tr.append(thresholds[np.where(recall > 0.90)[0][-1]])
